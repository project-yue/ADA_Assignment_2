...
public class Question1 extends Digraph implements ActionListener {
    ...
    private List<SelectedEdge> selectedEdges; //tree edge in dfs or the edge in the path of bfs
    private List<SelectedEdge> tempEdges; //edges that visited when do dfs research used for cycle check
    ...
    private ArrayList<ArrayList<Integer>> scc; //scc is used for record the each SCC with cycle
    ...
    private Color[] colorArray = {Color.RED, Color.MAGENTA, Color.PINK,
        ...
        Color.BLUE, Color.YELLOW, Color.GRAY, Color.ORANGE}; // used for coloring each SCC
    private static HashMap<Integer, Integer> parent; //record the parent node in distance determination
    ...
    private List<Edge> edgeList; //list of  all edges in the graph, for Eulerian trail search
        ...
        while (!bfsQueue.isEmpty()) {
            ...
            for (Integer adjacentNode : data.get(node)) {
                if (!visited.get(adjacentNode)) {
                    ...
                    searchForest.get(node).addChild(searchForest.get(adjacentNode));
    ...
    // perfoms depth first search on a digraph until all nodes are visited
    private void performDFSV2(Integer source) {
        visited = new HashMap<>();
        selectedEdges = new ArrayList<>();
        stack = new Stack<>();
        for (Integer node : nodeSet) {
    ...
    // depth first search from a node
    private void recursiveDFSV2(Integer node) {
        visited.put(node, Boolean.TRUE);

        for (Integer adjacentNode : data.get(node)) {
            if (!visited.get(adjacentNode)) {
                selectedEdges.add(new SelectedEdge(node, adjacentNode));
                recursiveDFSV2(adjacentNode);
            }
        }
    ...
    private void recursiveCycleCheck(Integer node) {
        ...
        for (Integer adjacentNode : data.get(node)) {
            ...
            } else if ((prevList.get(node) > prevList.get(adjacentNode)) && postList.get(adjacentNode) == -1) {
                tempEdges.add(new SelectedEdge(node, adjacentNode));
    ...
    private void performSCC(Integer source) {
        reverseGraph.performDFSV2(source);
        ...
        if (graph.outdegree(node) != 0) {
            for (Integer adjacentNode : graph.data.get(node)) {
                if (!graph.visited.get(adjacentNode)) {
                    graph.selectedEdges.add(new SelectedEdge(node, adjacentNode));
    ...
    public void performBFSDistance(Integer node1, Integer node2) {
        ...
        if (!visited.get(node2)) {
            System.out.println("The distance from node " + node1 + " to node " + node2 + " is infinity.");
        } else {
    ...
    public boolean isEulerian() {
        ...
        List<Integer> greaterOutdegree = new ArrayList<>(); //for nodes that outdegree is greater one than indegree
        List<Integer> greaterIndegree = new ArrayList<>(); //for nodes that indegree is greater one than outdegree
        ...
        if (((greaterOutdegree.size() == 1) && (greaterIndegree.size() == 1))
            ...
            if (trail.size() != graphSize()) {// the graph is not strongly connected
                return false;
            }
            for (Integer node : nodeSet) {//check if there is any stand alone vertext
                if (!visited.get(node)) {
                    return false;
                }
            }
        ...
        if (sccEnabled) {
            ...
            if (!isCycle) {//DAG               
                ...
                for (int index = 0; index < scc.size(); index++) {
                    ...
                    if (list.size() == 1) {
                        nodeList.get(list.get(0)).draw(g, colorArray[currentIndex]);
                    } else {
                        Question1 subGraph = new Question1();
                        for (Integer node : list) {
                            subGraph.add(node);
                        }
                        for (Integer j : list) {
                            List<Integer> adjlist = data.get(j);
                            for (int w : adjlist) {
                                if (list.contains(w)) {
                                    subGraph.addEdge(j, w);
                                }
                            }
                        }
                        subGraph.performDFSV2(list.get(0));
                        for (int i : list) {
                            nodeList.get(i).draw(g, colorArray[currentIndex]);
                        }
                        for (SelectedEdge selectedEdge : subGraph.selectedEdges) {
                            drawEdge(selectedEdge.getFromNode(), selectedEdge.getToNode(),
                                    g, colorArray[currentIndex], colorArray[currentIndex]);
                        }
            ...
            switch (token) {
                            ...
                            for (int i = 0; i < tempEdges.size(); i++) {
                                if (tempEdges.get(i).getFromNode() == firstCycleNode) {
                                    for (int j = i; j < tempEdges.size(); j++) {
                                        SelectedEdge selectedEdge = tempEdges.get(j);
                ...
                case "linearize":
                    try {
                        ...
                        if (isCycle == false) {
                            ...
                            System.out.println("The Graph has a cycle. It cannot be linearized!");
                            for (int i = 0; i < tempEdges.size(); i++) {
                                if (tempEdges.get(i).getFromNode() == firstCycleNode) {
                                    for (int j = i; j < tempEdges.size(); j++) {
                                        SelectedEdge selectedEdge = tempEdges.get(j);
                ...
                case "scc":
                    try {
                        ...
                        cycleCheck(node1);
                        if (isCycle == false) {
                            System.out.println("The graph is a DAG.There are " + graphOrder() + " SCC");
                            ...
                            System.out.print("{");
                            System.out.print(standaloneList.get(standaloneList.size() - 1) + "}");
                            System.out.println();
                            isPassEventToParent = false;
                        } else {
                            performSCC(node1);
                ...
                case "eulerian":
                    try {
                        ...
                        if (!isEulerianTrail) {
                            ...
                            while (trail.size() > 1) {
                                edge = trail.poll();
                                selectedEdges.add(new SelectedEdge(edge.getFromNode(), edge.getToNode()));
                                System.out.print(edge.getFromNode() + "->" + edge.getToNode() + ", ");
                            }
                            edge = trail.poll();
                            selectedEdges.add(new SelectedEdge(edge.getFromNode(), edge.getToNode()));
                            System.out.print(edge.getFromNode() + "->" + edge.getToNode());
